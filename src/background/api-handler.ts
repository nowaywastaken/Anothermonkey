    ...
    switch (action) {
        ...
        case "GM_xmlhttpRequest": {
            ...
            try {
                ...
                port.postMessage({ type: "error", error: { name: (error as Error).name, message: (error as Error).message } });
            } finally {
        ...
        case "GM_notification": {
            ...
            if (message.details.onclick || message.details.ondone || message.details.buttons) {
            ...
            const options: chrome.notifications.NotificationOptions = {
                type: message.details.buttons && message.details.buttons.length > 0 ? "basic" : "basic",
                iconUrl: message.details.imageUrl || chrome.runtime.getURL("assets/icon.png"),
                title: message.details.title || script?.metadata.name || "AnotherMonkey Notification",
                message: message.details.text || "",
                priority: 1,
                eventTime: message.details.timeout ? Date.now() + message.details.timeout : undefined
            };
            
            // Add buttons if provided (max 2 for chrome.notifications)
            if (message.details.buttons && message.details.buttons.length > 0) {
                options.buttons = message.details.buttons.slice(0, 2).map((btn: { title: string }) => ({
                    title: btn.title
                }));
            }
            ...
            try {
                ...
                if (message.details.timeout && message.details.timeout > 0) {
                    ...
                    }, Math.min(message.details.timeout, 30000)); // Max 30 seconds
                }
                
                return { success: true, notificationId };
            } catch (error) {
                return { error: `Failed to create notification: ${error instanceof Error ? error.message : String(error)}` };
            }
        }
        ...
        case "GM_download": {
            if (!await checkPermissions(scriptId, "GM_download")) return { error: "Missing permission: GM_download" };
            const { details, downloadId } = message;

            if (!await checkConnect(scriptId, details.url)) {
                const error = { name: "ConnectError", message: `Download from ${details.url} blocked by @connect rule.` };
                message.port.postMessage({ type: "error", error });
            ...
            try {
                const downloadOptions: chrome.downloads.DownloadOptions = {
                    ...
                    headers: details.headers ? Object.entries(details.headers).map(([name, value]) => ({ name, value: value as string })) : undefined,
                };

                const newDownloadId = await chrome.downloads.download(downloadOptions);
                port.postMessage({ type: "downloadStarted", downloadId: newDownloadId });
            } catch (error) {
                port.postMessage({ type: "error", error: { name: "DownloadError", message: error instanceof Error ? error.message : String(error) } });
        ...
        case "GM_cookie": {
            if (!await checkPermissions(scriptId, "GM_cookie")) {
                message.port.postMessage({ type: "error", error: { name: "PermissionError", message: "Missing permission: GM_cookie" } });
                return;
            }
            ...
            try {
                ...
                if (!details.url || !await checkConnect(scriptId, details.url)) {
                    port.postMessage({ type: "error", error: { name: "ConnectError", message: `Cookie access to ${details.url} blocked by @connect rule.` } });
                ...
                if (cookieAction === 'list') {
                    ...
                    if (cookie) {
                        port.postMessage({ type: "success", result: simplifyCookie(cookie) });
                    } else {
                        port.postMessage({ type: "error", error: { name: "CookieError", message: "Failed to set cookie" } });
                    }
                } else if (cookieAction === 'delete') {
                    // Delete a cookie by name
                    if (!details.name) {
                        port.postMessage({ type: "error", error: { name: "CookieError", message: "Cookie name is required for delete action" } });
                        cookiePorts.delete(cookieId);
                        return;
                    }
                    ...
                    if (removed) {
                        port.postMessage({ type: "success", result: { success: true, name: removed.name } });
                    } else {
                        port.postMessage({ type: "error", error: { name: "CookieError", message: "Cookie not found or could not be deleted" } });
                    }
                } else {
                    port.postMessage({ type: "error", error: { name: "CookieError", message: `Unknown cookie action: ${cookieAction}` } });
                }
            } catch (error: unknown) {
                port.postMessage({ type: "error", error: { name: "CookieError", message: error instanceof Error ? error.message : String(error) } });
            } finally {
                cookiePorts.delete(cookieId);
            }
            return;
