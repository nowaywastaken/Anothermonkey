...
chrome.runtime.onMessage.addListener(
  (message: unknown, sender, sendResponse) => {
    ...
    if (msg.action === "execute_menu_command") {
      const { targetTabId, commandId } = msg as {
        targetTabId: number;
        commandId: string;
      };
...
// Initialize downloads API event listeners safely
function initializeDownloadsAPI() {
  if (!chrome.downloads) {
    logger.warn("Downloads API not available");
    return;
  }
  ...
  try {
    ...
    logger.error("Failed to initialize downloads API:", error);
  }
}
...
chrome.contextMenus.onClicked.addListener(
  (info: chrome.contextMenus.OnClickData, tab) => {
    if (
      !tab?.id ||
      !(typeof info.menuItemId === "string") ||
      !info.menuItemId.startsWith("anmon-cmd::")
    )
      return;
    ...
    if (commandId) {
      chrome.tabs
        ...
        .catch((e) =>
          logger.debug(
            "Could not send menu click to tab, it might have been closed.",
            e,
          ),
        );
    }
...
chrome.notifications.onClicked.addListener(async (notificationId) => {
  ...
  if (
    notificationId.startsWith("connect-blocked::") ||
    notificationId.startsWith("download-connect-blocked::")
  ) {
    ...
    chrome.tabs.create({
      url: `tabs/permission.html?scriptId=${scriptId}&domain=${domain}`,
    });
  ...
  if (notificationId.startsWith("gm-notification::")) {
    ...
    if (notifInfo?.onclick && notifInfo?.tabId) {
      ...
      chrome.tabs
        .sendMessage(notifInfo.tabId, {
          action: "GM_notificationClick",
          notificationId: notificationId.split("::").pop(),
        })
        .catch((e) =>
          logger.debug("Could not send notification click to tab", e),
        );
    }
...
chrome.notifications.onButtonClicked.addListener(
  async (notificationId, buttonIndex) => {
    if (
      ...
      !notificationId.startsWith("gm-notification::")
    )
      return;
    ...
    if (notifInfo?.buttons && notifInfo?.tabId) {
      ...
      chrome.tabs
        ...
        .catch((e) =>
          logger.debug("Could not send notification button click to tab", e),
        );
    }
...
chrome.notifications.onClosed.addListener(async (notificationId, byUser) => {
  ...
  if (notifInfo?.ondone && notifInfo?.tabId) {
    ...
    chrome.tabs
      ...
      .catch((e) =>
        logger.debug("Could not send notification close to tab", e),
      );
  }
...
async function setupUpdateAlarm() {
  ...
  const intervalMinutes =
    (result.updateCheckIntervalMinutes as number) ||
  ...
  (message: unknown, sender, sendResponse) => {
    ...
    if (msg.action === "configure_update_check" && msg.intervalMinutes) {
      chrome.storage.local
        .set({ updateCheckIntervalMinutes: msg.intervalMinutes })
        .then(() => {
          setupUpdateAlarm();
  ...
  for (const script of scripts) {
    try {
      await chrome.userScripts.configureWorld({
        worldId: script.id,
        messaging: true,
        csp: "script-src 'self' 'unsafe-inline' 'unsafe-eval' blob: https://cdn.jsdelivr.net; object-src 'none'; script-src-attr 'none'; connect-src 'self' https://cdn.jsdelivr.net",
      });
    } catch (e) {
