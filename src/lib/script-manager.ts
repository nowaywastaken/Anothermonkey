...
async function buildScriptPayload(
  script: UserScript,
): Promise<{ js: { code: string }[]; world: "MAIN" | "USER_SCRIPT" }> {
  const isGrantNone =
    script.metadata.grants.length === 0 ||
    (script.metadata.grants.length === 1 &&
      script.metadata.grants[0] === "none");

  const world =
    isGrantNone || script.preferredWorld === "MAIN" ? "MAIN" : "USER_SCRIPT";
  ...
  const needsPreCheck =
    script.metadata.includes.length > 0 ||
    script.metadata.excludes.length > 0 ||
    script.metadata.noframes;
  ...
  if (needsPreCheck) {
    ...
    const noframesCheck = script.metadata.noframes
...
export async function syncScripts() {
  ...
  try {
    ...
    const scriptsToAdd = dbScripts.filter(
      (s) => !registeredScriptIds.has(s.id),
    );
    const scriptsToUpdate = dbScripts.filter((s) =>
      registeredScriptIds.has(s.id),
    );
    ...
    const buildRegistration = async (
      script: UserScript,
    ): Promise<chrome.userScripts.RegisteredUserScriptOptions> => {
      ...
      const includesAsMatches = script.metadata.includes.filter(
      ...
      const registration: chrome.userScripts.RegisteredUserScriptOptions = {
        ...
        matches,
        excludeMatches: script.metadata.excludes.filter(
    ...
    logger.info(
      `Sync complete. Added: ${scriptsToAdd.length}, Updated: ${scriptsToUpdate.length}, Removed: ${scriptIdsToRemove.length}.`,
    );
  } catch (error) {
...
export async function injectIntoExistingTabs(script: UserScript) {
  ...
  for (const tab of allTabs) {
    ...
    const shouldRunOnTab =
      (patternsToMatch.length === 0 || matchesUrl(patternsToMatch, tab.url)) &&
      !matchesUrl(patternsToExclude, tab.url);
    ...
    try {
      ...
      if (
        ...
        logger.error(
          `Failed to inject script "${script.metadata.name}" into tab ${tab.id}:`,
          e,
        );
      }
    }
...
export async function checkScriptUpdate(
  ...
  try {
    // Verify the update URL is from a trusted source
    const updateUrlObj = new URL(updateUrl);
    if (!["https:", "http:"].includes(updateUrlObj.protocol)) {
      logger.warn(
        `Update URL uses unsupported protocol: ${updateUrlObj.protocol}`,
      );
      return null;
    }
    ...
    const expectedHash = ScriptIntegrity.extractMetadataHash(code);
    const integrityResult = await ScriptIntegrity.performIntegrityCheck(
      code,
      script.lastHash || undefined,
      expectedHash || undefined,
    );

    // Log security warnings
    if (integrityResult.securityWarnings.length > 0) {
      logger.warn(
        `Security warnings for ${script.metadata.name}:`,
        integrityResult.securityWarnings,
      );
    }

    if (integrityResult.integrityWarnings.length > 0) {
      logger.warn(
        `Integrity warnings for ${script.metadata.name}:`,
        integrityResult.integrityWarnings,
      );
    }

    // If there are serious security issues, reject the update
    if (!integrityResult.valid) {
      logger.error(
        `Update rejected for ${script.metadata.name} due to security/integrity issues`,
      );
      return null;
    }
    ...
    if (isNewerVersion(newMetadata.version, script.metadata.version)) {
      return {
        ...
        newCode: code,
        newMetadata,
        newHash: integrityResult.actualHash,
      };
    }
  } catch (e) {
